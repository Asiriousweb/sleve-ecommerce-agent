...
| Telegram (reporte) | 🔴 | **Causa raíz (2026-06-29):** el servicio E-commerce en Railway tiene el `TELEGRAM_BOT_TOKEN` **del bot de Trade**, no el suyo → mensajes llegan a "SLEVE Trade Agent" + si ambos servicios comparten token, **409 Conflict → crash**. El token correcto (en `secrets/.env`) = bot **@Sleve_ecommerce_bot** "SLEVE Ecommerce Agent", chat `920578167`. **Fix: en Railway E-commerce, poner el token de @Sleve_ecommerce_bot (cada servicio su propio token).** Control en lenguaje natural = Fase 2: `bot._handle` solo comandos; texto libre → `orchestrator.fallback`. Falta cablear `orchestrator.ask` con Claude (créditos Anthropic) |
