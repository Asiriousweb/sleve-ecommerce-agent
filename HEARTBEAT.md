...
| **Mercado Libre (directo)** | 🟡 | **Conector LISTO y desplegado** (2026-06-30): OAuth `meli_oauth.py` (authorization_code + refresh 6h, 1 cuenta/país, país vía /users/me), `pull_meli` (ventas+pedidos+publicaciones por país en el rango del período) → `paises[].meli`; rutas /meli·/install·/callback; dashboard Canales → sección "Mercado Libre (directo)". `/meli` LIVE. **Pendiente del usuario:** crear app en ML developers (redirect `…/meli/callback`, scopes read+offline_access) → setear `MELI_CLIENT_ID`/`MELI_CLIENT_SECRET` en Railway → conectar cada país en /meli. Reemplaza Nubimetrics (~$320/mes); se cuadrará con Multivende |
