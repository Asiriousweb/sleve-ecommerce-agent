...
| **Search Console** | 🟡 | DIRECTO listo (`pull_search_console`, misma service account). `SC_SITE_CL/CO/MX/PE = sc-domain:sleve.X` ya en Railway. **Bloqueo: 403 "insufficient permissions"** → falta agregar el `client_email` de la SA como usuario (Restringido) en las 4 propiedades de dominio `sleve.X` (GA4 y SC son sistemas separados). Sitio activo confirmado = `sleve.X` (dominio primario del Shopify de Chile). 2026-06-29 |
