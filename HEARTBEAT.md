...
| Telegram (reporte) | 🔴 | **Diagnóstico 2026-06-29:** no llega nada ni responde → casi seguro `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` NO están como **env vars en Railway** (solo en `secrets/.env`, que es gitignored → no se deploya). Sin token, `bot.py` se cae al arrancar. **Fix: cargar ambas en Railway.** Control en lenguaje natural ("hablarle para avanzar") = Fase 2 pendiente: `bot._handle` solo comandos fijos; texto libre → `orchestrator.fallback` (enlatado). Falta cablear `orchestrator.ask` con Claude (necesita créditos Anthropic) |
