...
| **MCP remoto (read-only)** | 🟢→🟡 | **Endpoint LIVE y validado** end-to-end (POST `/mcp`, JSON-RPC a mano en run_railway.py, sin deps). 7 tools de lectura (resumen_global, ventas_por_pais, adquisicion, publicaciones, tendencias, cuadratura, acciones) que leen overview.json por período. Probado: initialize + tools/list + tools/call OK en la URL de Railway. **Pendiente del usuario:** (1) setear `MCP_TOKEN` en Railway (hoy abierto), (2) crear subdominio `mcp-ecommerce.slevemobile.com` (CNAME → Railway custom domain), (3) conectarlo en Claude (Connectors → URL `/mcp` + bearer). Fase control (acciones con confirmación): futura |
