...
🟢 **OPERATIVO** (2026-07-01) — Dashboard LIVE con datos reales en https://ecommerce.slevemobile.com (robot Railway + Vercel), **protegido con PIN** (middleware Next + `DASH_PIN`). Conexiones directas (gratis): Shopify (6 tiendas), Meta Ads, Google Ads (4/4), GA4, Search Console, Klaviyo (4), Redes orgánico (FB/IG), **YouTube** (Data API v3), **Metricool** (hub social — engagement por post), Google Trends, **Mercado Libre 3/4**, Telegram, **MCP remoto**. Período = filtro global (7d/30d/mes/YoY). Consolidado en USD con cuadratura. **Reporte semanal por email** (lunes 8:00 Chile, 5 correos por país en moneda local + consolidado USD, evolución sem-vs-sem + mes YoY). Regla de fuentes social: directo = verdad, Metricool corrobora/rellena.

**Pendiente** (ver `TASKS.md`/`CONNECTIONS.md`): Mercado Libre Colombia (verificación ML), Multivende (corazón marketplaces, esperando credenciales), **Walmart 3P directo** (MX), TikTok Ads, Gorgias, Business Profile, Merchant Center, backlog social/ads (LinkedIn, Amazon, Threads directo, X, Spotify), créditos Anthropic (loop nocturno + control natural), P&L con COGS/comisiones, completar `CONTEXT.md`/`DATA.md`.
